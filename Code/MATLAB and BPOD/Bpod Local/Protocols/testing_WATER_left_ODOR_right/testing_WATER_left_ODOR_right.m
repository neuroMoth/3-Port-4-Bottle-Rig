...
function testing_WATER_left_ODOR_right
    ...
    for trial= 1:expV.MAXIMUM_TRIALS
        ...
        %fprintf(['Blanktimes:',num2str(blankOpenTimes(trial,1)),',',num2str(blankOpenTimes(trial,2)),',',num2str(blankOpenTimes(trial,3))])
        ...
        sma = AddState(sma, 'Name', 'waitForRemainingCenterDryLicks', ...
            'Timer', 0,...
            'StateChangeConditions', {center_port.LEFT_COUNTER_EVENT, 'waitCenterRewardLick1', expV.experimentTimeExpired , 'cleanup', expV.lickTimeExpired , 'reportSkip'},...
            'OutputActions',{center_port.DOOR, expV.DOWN});

        sma = AddState(sma, 'Name', 'waitCenterRewardLick1', ...
            'Timer', 0,...
            'StateChangeConditions', {center_port.LEFT_LICK_INPUT, 'openCenterValve', expV.experimentTimeExpired , 'cleanup', expV.lickTimeExpired , 'reportSkip' },...
            'OutputActions',{center_port.DOOR, expV.DOWN});

        sma = AddState(sma, 'Name', 'openCenterValve', ...
            'Timer', blankOpenTimes(trial,1),...
            'StateChangeConditions', {'Tup', 'closeCenterBlank1'},...
            'OutputActions',{center_port.DOOR, expV.DOWN, 'ValveModule1', ['O' center_port.left_valve], 'BNC1', 1});

         sma = AddState(sma, 'Name', 'closeCenterBlank1', ...
            'Timer', (center_port.left_valve_time-blankOpenTimes(trial,1)),...
        ...
        sma = AddState(sma, 'Name', 'waitCenterRewardLick2', ...
            'Timer', 0,...
            'StateChangeConditions', {center_port.LEFT_LICK_INPUT, 'openCenterValve2', expV.experimentTimeExpired , 'cleanup', expV.lickTimeExpired , 'reportSkip' },...
            'OutputActions',{center_port.DOOR, expV.DOWN});

        sma = AddState(sma, 'Name', 'openCenterValve2', ...
            'Timer', blankOpenTimes(trial,2),...
            'StateChangeConditions', {'Tup', 'closeCenterBlank2', expV.experimentTimeExpired , 'cleanup'},...
            'OutputActions',{center_port.DOOR, expV.DOWN, 'ValveModule1', ['O' center_port.left_valve], 'BNC1', 1});

        sma = AddState(sma, 'Name', 'closeCenterBlank2', ...
            'Timer', (center_port.left_valve_time-blankOpenTimes(trial,2)),...
            'StateChangeConditions', {'Tup', 'centerValveOff2'},...
            'OutputActions',{center_port.DOOR, expV.DOWN,'BNC1', 0});
        ...
        %     'Timer', 0,...
        %     'StateChangeConditions', {center_port.LEFT_LICK_INPUT, 'ttcLateralTimeout', expV.experimentTimeExpired , 'cleanup', expV.lickTimeExpired , 'reportSkip'},...
        %     'OutputActions',{center_port.DOOR, expV.DOWN});

        % Adding a sixth reward lick to make sure they get enough to sample - TVD 9/25/2025
        ...
        sma = AddState(sma, 'Name', 'waitCenterRewardLick3', ...
            'Timer', 0,...
            'StateChangeConditions', {center_port.LEFT_LICK_INPUT, 'openCenterValve3', expV.experimentTimeExpired , 'cleanup', expV.lickTimeExpired , 'reportSkip' },...
            'OutputActions',{center_port.DOOR, expV.DOWN});

        sma = AddState(sma, 'Name', 'openCenterValve3', ...
            'Timer', blankOpenTimes(trial,3),...
            'StateChangeConditions', {'Tup', 'closeCenterBlank3'},...
            'OutputActions',{center_port.DOOR, expV.DOWN, 'ValveModule1', ['O' center_port.left_valve], 'BNC1', 1});

        sma = AddState(sma, 'Name', 'closeCenterBlank3', ...
            'Timer', (center_port.left_valve_time-blankOpenTimes(trial,3)),...
        ...
        sma = AddState(sma, 'Name', 'waitLateralDryLicks', ...
            'Timer', 0,...
            'StateChangeConditions', {correct_port.lick_counter_event, 'waitLateralRewardLick1', incorrect_port.lick_counter_event, ...
        ...
        sma = AddState(sma, 'Name', 'openLateralReward1', ...
            'Timer', blankOpenTimes(trial,4),...
            'StateChangeConditions', {'Tup', 'closeLateralBlank1'},...
            'OutputActions',{port_1.DOOR, expV.DOWN, port_3.DOOR, expV.DOWN, 'ValveModule1', ['O', correct_port.valve], 'BNC1', 1});

        sma = AddState(sma, 'Name', 'closeLateralBlank1', ...
            'Timer', (correct_port.valve_time-blankOpenTimes(trial,4)),...
        ...
        sma = AddState(sma, 'Name', 'openLateralReward2', ...
            'Timer', blankOpenTimes(trial,5),...
            'StateChangeConditions', {'Tup', 'closeLateralBlank2'},...
            'OutputActions',{port_1.DOOR, expV.DOWN, port_3.DOOR, expV.DOWN, 'ValveModule1', ['O', correct_port.valve], 'BNC1', 1});

        sma = AddState(sma, 'Name', 'closeLateralBlank2', ...
            'Timer', (correct_port.valve_time-blankOpenTimes(trial,5)),...
        ...
        sma = AddState(sma, 'Name', 'openLateralReward3', ...
            'Timer', blankOpenTimes(trial,6),...
            'StateChangeConditions', {'Tup', 'closeLateralBlank3'},...
            'OutputActions',{port_1.DOOR, expV.DOWN, port_3.DOOR, expV.DOWN, 'ValveModule1', ['O', correct_port.valve], 'BNC1', 1});

        sma = AddState(sma, 'Name', 'closeLateralBlank3', ...
            'Timer', (correct_port.valve_time-blankOpenTimes(trial,6)),...
