...
function side_training_day_1_odor_right
    ...
    for trial= 1:expV.MAXIMUM_TRIALS
        ...
        if (trial == 1)
            %%% change this to 2, 5. direct from center to lateral, not lateral to center
            %% valves 2-4 are WATER, and valves 5-7 are ODOR
            CENTER_VALVES = [2, 5];

            first_stimulus = CENTER_VALVES(randi(2));
             
            center_port = center_port.setValve(1, first_stimulus);

            [correct_port, center_port] = correct_port.setCorrect(port_1, center_port, port_3, first_stimulus);
            incorrect_port = incorrect_port.setIncorrect(port_1, center_port, port_3, first_stimulus);
        ...
        if (BpodSystem.Status.switchStimulusFlag)
            ...
            center_port = center_port.switchValve;

            [correct_port, center_port] = correct_port.setCorrect(port_1, center_port, port_3, center_port.left_valve);
            incorrect_port = incorrect_port.setIncorrect(port_1, center_port, port_3, center_port.left_valve);
        ...
        if (trial > expV.MINIMUM_TRIALS) % evaluate if minimum trial number is reached, and if 10 consecutive traials have been skipped
            if(BpodSystem.Status.consecutiveRatSkips >= 2)
                stop_experiment(A);
                return
            end
        end
        ...
        sma = AddState(sma, 'Name', 'waitLateralDryLicks', ...
            ...
            'StateChangeConditions', {correct_port.lick_counter_event, 'waitLateralRewardLick1', incorrect_port.lick_counter_event, 'punish', expV.experimentTimeExpired , 'cleanup'...
                ...
                'OutputActions',{port_1.DOOR, expV.DOWN, port_3.DOOR, expV.DOWN, 'GlobalTimerTrig', expV.lickWindowTimerID});
            ...
            sma = AddState(sma, 'Name', 'waitLateralRewardLick1', ...
                ...
                sma = AddState(sma, 'Name', 'closeLateralReward1', ...
                    ...
                    'OutputActions',{port_1.DOOR, expV.DOWN, port_3.DOOR, expV.DOWN, 'ValveModule1', ['C', correct_port.valve], 'BNC1', 0});
                ...
                sma = AddState(sma, 'Name', 'waitLateralRewardLick2', ...
                    ...
                    sma = AddState(sma, 'Name', 'waitLateralRewardLick3', ...
                        ...
                        sma = AddState(sma, 'Name', 'punish', ...
                            ...
                            'OutputActions',{port_1.DOOR, expV.UP, port_3.DOOR, expV.UP, 'SoftCode', 2});
