...
function lick_training_familiarization_CENTER 
    ...
    for trial= 1:expV.MAXIMUM_TRIALS
        ...
        sma = AddState(sma, 'Name', 'centerPortDown', ...
            'Timer', 0,...
            'StateChangeConditions', {port_1.LICK_INPUT, 'noRewardLick', 'GlobalTimer1_End', 'cleanup'},...
            'OutputActions',{port_1.DOOR, expV.DOWN});

        sma = AddState(sma, 'Name', 'noRewardLick', ...
            'Timer', 0,...
            'StateChangeConditions', {port_1.LICK_INPUT, 'RewardLick', 'GlobalTimer1_End', 'cleanup'},...
            'OutputActions',{port_1.DOOR, expV.DOWN});

        sma = AddState(sma, 'Name', 'RewardLick', ...
            'Timer', port_1.VALVE_TIME,...
            'StateChangeConditions', {'Tup', 'flipValveOff', 'GlobalTimer1_End', 'cleanup'},...
            'OutputActions',{port_1.DOOR, expV.DOWN, 'ValveModule1', ['O', port_1.VALVE], 'BNC1', 1});

        sma = AddState(sma, 'Name', 'flipValveOff', ...
            'Timer', 0,...
            'StateChangeConditions', {'Tup', 'centerPortDown', 'GlobalTimer1_End', 'cleanup'},...
            'OutputActions',{port_1.DOOR, expV.DOWN, 'ValveModule1', ['C', port_1.VALVE], 'BNC1', 0});

        sma = AddState(sma, 'Name', 'cleanup', ...
            'Timer', 0,...
            'StateChangeConditions', {'Tup', 'exit'},...
            'OutputActions',{port_1.DOOR, expV.UP, 'ValveModule1', ['C', port_1.VALVE]});
