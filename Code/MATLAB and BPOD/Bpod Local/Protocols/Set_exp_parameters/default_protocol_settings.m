...
protocol_settings.GUIPanels.Liquid_Amounts_ul = {};
for i = 1:Set_param_constants.NUM_VALVES
    line = i;

    variable_name =  sprintf('select_amount_valve%d', line);
    %protocol_settings = create_popup_menu(protocol_settings, variable_name, 21, stimulus_options);
    protocol_settings = create_popup_menu(protocol_settings, variable_name, 7, liquid_amount_options);

    protocol_settings.GUIPanels.Liquid_Amounts_ul{end + 1} = variable_name;
end
%%% END PANEL %%%
