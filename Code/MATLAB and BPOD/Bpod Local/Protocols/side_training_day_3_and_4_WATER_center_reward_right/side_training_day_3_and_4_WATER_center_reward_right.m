...
% SIDE TRANING DAYS 3-4 | ODOR CENTER | VALVE 5 CENTER | RIGHT PORT REWARD
...
function side_training_day_1_odor_right
    ...
    LATERAL_INPUT = PORT_3_INPUT;
    LATERAL_DOOR = PORT_3_DOOR;
    LATERAL_LICK_COUNTER = PORT_3_COUNT_NUM;
    LATERAL_COUNTER_COMPLETE = PORT_3_COUNT_COMPLETE;
    ...
    while true
        ...
        sma = AddState(sma, 'Name', 'waterRewardLick', ...
            'Timer', 0,...
            'StateChangeConditions', {'Tup', 'beginCenterValve', 'GlobalTimer1_End', 'cleanup', CENTER_LICK_COMPLETE, 'lateralTTC_Timeout'},...
            'OutputActions',{LATERAL_DOOR, UP, CENTER_PORT_DOOR, DOWN});
        ...
        % lateralTTC_Timeout introduces a 3 second delay from the time the center port door begins moving up to the time 
        % the lateral door begins to open. 
        % State change conditions include: 
        %   - tup -> move to 'TTC_Port_1' state after 3 second timeout 

        % OutputActions
        %   - All doors remain closed
        sma = AddState(sma, 'Name', 'lateralTTC_Timeout', ...
            'Timer', 3,...
            'StateChangeConditions', {'Tup', 'TTC_Lateral', 'GlobalTimer1_End', 'cleanup'},...
        ...
        sma = AddState(sma, 'Name', 'odorRewardLick', ...
            'Timer', 0,...
            'StateChangeConditions', {'Tup', 'beginLateralValve', 'GlobalTimer1_End', 'cleanup', LATERAL_COUNTER_COMPLETE, 'rewardLick6_ITI'},...
            'OutputActions',{LATERAL_DOOR, DOWN, CENTER_PORT_DOOR, UP});
