classdef PortHandler
    ...
    methods
        function obj = setCorrect(obj, port_1_inst, port_3_inst, center_valve)
            switch center_valve
                % for this script, water valves are rewarded on the RIGHT (port 3), 
                % while odor valves are rewarded on the LEFT (port 1) 
                case {2, 3, 4} 
        ...
        function obj = setIncorrect(obj, port_1_inst, port_3_inst, center_valve)
            switch center_valve
                % if correct is 3, incorrect is the opposite
                case {2, 3, 4} % water valves correct port -> RIGHT (PORT 3) | incorrect -> LEFT (PORT 1)
                    obj = obj.setProperties(1, port_1_inst);
                case {5, 6, 7} % odor valves correct port -> LEFT (PORT 1) | incorrect -> RIGHT (PORT 3)
                    obj = obj.setProperties(3, port_3_inst);
            end
        end
