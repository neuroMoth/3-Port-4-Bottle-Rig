...
function alternation_shuffled_nonconsecutive_WATER_left_ODOR_right
    global BpodSystem
    ...
    BpodSystem.Data.lateralPortChoice = zeros(expV.MAXIMUM_TRIALS, 1);
    ...
    % added by TVD to randomly select the number of nonconsecutive corrects to switch the center stimulus (for the first trial)
    BpodSystem.Status.correct_required_to_switch = expV.CORRECT_REQUIRED_TO_SWITCH(1,randi(length(expV.CORRECT_REQUIRED_TO_SWITCH)));
    ...
    for trial= 1:expV.MAXIMUM_TRIALS
        ...
        if (BpodSystem.Status.switchStimulusFlag)
            ...
            % added by TVD to randomly select the number of nonconsecutive corrects to switch the center stimulus
            BpodSystem.Status.correct_required_to_switch = expV.CORRECT_REQUIRED_TO_SWITCH(1,randi(length(expV.CORRECT_REQUIRED_TO_SWITCH)));

            fprintf('Correct=port%d. ',correct_port.port)
            fprintf('Incorrect=port%d. ',incorrect_port.port)
            fprintf('%d correct to switch. ',BpodSystem.Status.correct_required_to_switch)
        end
