...
% SIDE TRANING DAY ONE | WATER CENTER | VALVE 2 CENTER | RIGHT PORT REWARD 
...
function side_training_day_1_odor_right
    ...
    for trial= 1:expV.MAXIMUM_TRIALS
        ...
        if (trial == 1)
            first_stimulus_valve = center_port.WATER_VALVE;
        ...
        sma = AddState(sma, 'Name', 'centerValveOff2', ...
            'Timer', 0,...
            'StateChangeConditions', {'Tup', 'waitRemainingCenterLicks', expV.experimentTimeExpired , 'cleanup'},...
            'OutputActions',{center_port.DOOR, expV.DOWN, 'ValveModule1', ['C' center_port.left_valve], 'BNC1', 0});
        ...
        sma = AddState(sma, 'Name', 'ttcLateral', ...
            ...
            'OutputActions',{port_3.DOOR, expV.DOWN, 'GlobalCounterReset', port_1.COUNTER_ID});
        ...
        sma = AddState(sma, 'Name', 'waitLateralRewardLick1', ...
            ...
            'OutputActions',{port_3.DOOR, expV.DOWN, 'GlobalTimerTrig', expV.lickWindowTimerID});

        sma = AddState(sma, 'Name', 'openLateralReward1', ...
            'Timer', correct_port.valve_time,...
            'StateChangeConditions', {'Tup', 'closeLateralReward1'},...
            'OutputActions',{port_3.DOOR, expV.DOWN, 'ValveModule1', ['O', correct_port.valve], 'BNC1', 1});

        % end lick two, wait for lick 3 (dry) 
        sma = AddState(sma, 'Name', 'closeLateralReward1', ...
            'Timer', 0,...
            'StateChangeConditions', {correct_port.lick_event, 'waitLateralRewardLick2', expV.lickTimeExpired, 'exit'},...
            'OutputActions',{port_3.DOOR, expV.DOWN, 'ValveModule1', ['C', correct_port.valve], 'BNC1', 0});
        ...
        sma = AddState(sma, 'Name', 'closeLateralReward2', ...
            'Timer', 0,...
            'StateChangeConditions', {correct_port.lick_event, 'waitLateralRewardLick3', expV.lickTimeExpired, 'exit'},...
            'OutputActions',{port_3.DOOR, expV.DOWN, 'ValveModule1', ['C', correct_port.valve], 'BNC1', 0});
        ...
        sma = AddState(sma, 'Name', 'punish', ...
            ...
            'OutputActions',{port_3.DOOR, expV.UP, 'SoftCode', 2});
